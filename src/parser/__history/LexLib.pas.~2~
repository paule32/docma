...
var
  ...
  eoi: Integer;
  bufptr: Integer;
  ungetbuf: array [0 .. max_chars] of AnsiChar;
  ungetbufptr: Integer;
...
begin
  ...
  if (bufptr <= eoi) then
  begin
    get_char := AnsiChar(yyinput_text[bufptr]);
    Inc(bufptr);
    if (Result = #13) then
      Result := ' ';
  end
...
begin
  ...
  begin
    ...
    yypos[n] := 0;
    if yyleng > 0 then
      yylastchar := AnsiChar(yytext[yyleng])
    else
      yylastchar := #0;
    yyfind := true;
  end
